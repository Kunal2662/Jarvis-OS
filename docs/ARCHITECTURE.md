...
| MCP Architecture | 🟡 Partial | **M10.5** (MCP & Integration Platform) | `core/mcp/` — Capability Registry, client/server runtimes, negotiation, heartbeat; `core/mcp/transports/` — stdio/websocket/http/ipc + factory; `core/interfaces/mcp.py` (ports). Task Groups A+B shipped; no provider integration yet |
