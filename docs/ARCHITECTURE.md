...
| Workspace Architecture | 🟡 Partial | **M11** (Intelligent Workspace & Productivity, Task Group A shipped) | `domain/workspace/` — `WorkspaceSettings` (JSON column) + `WorkspaceMetadata` (derived, never stored); `infrastructure/database/models.py` — `Workspace`/`Project`/`Note`; `repositories/workspace_repository.py` — three repositories; `services/workspace_service.py` — the domain; `services/workspace_manager.py` — composition with Knowledge/Search/Memory; `routes/workspaces.py` — CRUD + `/overview` + `/context`. Task Groups B–F (Tasks/Calendar, Files, AI context, integrations, UI) not started |
