...
| Voice String behavior | No Orb, no visible state label ("Listening...") — a continuous animated wave whose color/speed/amplitude communicate Idle/Wake/Listening/Thinking/Speaking/Success/Error, backed by a real validated state machine (`core/voice-state-machine.ts`) that starts and stays idle until a real voice backend exists. Respects `useReducedMotion()` directly (freezes rather than animates), since the wave is a continuous `useTime()` loop, not a discrete transition `MotionConfig`'s app-wide setting already covers (added Aug 2026, M8 Phase 4 Task Group H). |
