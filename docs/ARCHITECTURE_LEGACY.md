# Architecture — Legacy (M0–M7, PySide6 era)

> **Historical reference.** This document describes the architecture
> actually shipped for M0–M7, on the PySide6 desktop UI. It is
> preserved verbatim (renamed from `ARCHITECTURE.md`, Aug 2026) and is
> not updated as new milestones ship. For the current, forward-looking
> architecture standard (M8 onward, React + Tauri + FastAPI), see
> [`ARCHITECTURE.md`](ARCHITECTURE.md). Every layer below `UI` in this
> document's own layer diagram — `Features` down through
> `Infrastructure` — remains accurate today; only the `UI` layer's
> technology changed. See [`TECH_STACK.md`](TECH_STACK.md) for the
> migration.
