...
def build_thermostat_tools(thermostats: ThermostatService) -> list[BaseTool]:
    ...
    async def set_thermostat_state(
        ...
        *,
        ...
        mode, its swing mode, its preset mode, or any combination, in
        one call. Supply at least one of them. Temperature is in the
        device's own unit -- no conversion is performed. hvac_mode/
        fan_mode/swing_mode/preset_mode must each be one the device
        ...
        try:
            result = await thermostats.set_thermostat_state(
                ...
                preset_mode=preset_mode or None,
            )
        except Exception as err:
