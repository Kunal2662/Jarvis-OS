"""Smart Switches REST tests -- Milestone 12 Energy Management (Core
Energy Slice).
...
def _connected_switch(client, auth, fake_connector, home_id: str) -> str:
    ...
    fake_connector.devices = [
        DiscoveredDevice(
            external_id=f"switch.{home_id[:8]}", name="Living Room Plug", device_type="switch"
        )
    ]
...
def test_every_route_requires_a_session(client) -> None:
    for method, path in (("get", "/api/v1/switches"), ("get", "/api/v1/switches/x")):
...
def test_responses_use_the_documented_envelope(client, auth, fake_connector) -> None:
    ...
    _connected_switch(client, auth, fake_connector, home_id)
    listed = client.get("/api/v1/switches", headers=auth)
...
def test_list_and_get_do_not_require_permission_grant(client, auth, fake_connector) -> None:
    from jarvis.core.interfaces.connectivity import DeviceState

    home_id = _home(client, auth)
    device_id = _connected_switch(client, auth, fake_connector, home_id)
    ...
    fetched = client.get(f"/api/v1/switches/{device_id}", headers=auth)
    assert fetched.status_code == 200
    assert fetched.json()["data"]["id"] == device_id


def test_get_unknown_switch_is_404(client, auth) -> None:
    response = client.get("/api/v1/switches/no-such-device", headers=auth)
    assert response.status_code == 404


def test_get_switch_on_non_switch_device_is_404(client, auth) -> None:
    home_id = _home(client, auth)
    lock = client.post(
        "/api/v1/devices",
    ...
    response = client.get(f"/api/v1/switches/{lock['id']}", headers=auth)
...
def test_turn_on_denied_without_grant_is_400(client, auth, fake_connector) -> None:
    ...
    response = client.post(f"/api/v1/switches/{device_id}/on", headers=auth)
...
def test_turn_off_denied_without_grant_is_400(client, auth, fake_connector) -> None:
    ...
    response = client.post(f"/api/v1/switches/{device_id}/off", headers=auth)

    assert response.status_code == 400
    assert fake_connector.sent_commands == []


def test_turn_on_and_off_succeed_after_grant(client, auth, fake_connector) -> None:
    home_id = _home(client, auth)
    device_id = _connected_switch(client, auth, fake_connector, home_id)
    _grant_smart_home_permission(client, auth)

    turned_on = client.post(f"/api/v1/switches/{device_id}/on", headers=auth)
    assert turned_on.status_code == 200
    assert turned_on.json()["data"]["success"] is True
    assert turned_on.json()["meta"]["success"] is True

    turned_off = client.post(f"/api/v1/switches/{device_id}/off", headers=auth)
...
def test_turn_on_unknown_device_is_400(client, auth) -> None:
    response = client.post("/api/v1/switches/no-such-device/on", headers=auth)
    assert response.status_code == 400
